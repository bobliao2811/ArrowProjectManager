/*
*arrowHotLoader AMD规范异步请求资源框架 开发环境模块热更新组件
*此组件接收来自工作流管理器的开发环境模块的更新
*用于开发时的模块无刷新更新
*廖力编写于 2019/11/07
*/
var arrowHotLoader = function(){
	var self = this;
	if(typeof window.arrowConfig === 'undefined'){
		console.log('错误:arrowConfig配置文件不存在!已中断arrowHotLoader的初始化!');
		return;
	}
	...
	this.onOpen = function() {
        console.log("hotLoader:连接已经打开。");
    };
    this.onClose = function() {
        console.log("hotLoader:连接已经关闭");
    };
    this.onMessage = function(_data) {
       console.log('文件已经更改：');
       console.log(_data);
       self.handleHotLoad(_data.data);
    };
    this.onError = function(_message) {
        console.log("hotLoader:出现错误.");
	...
	this.createSocket = function(_onOpen,_onClose,_onError,_onMessage){
		if(this.config.arrow_hotLoaderPor === ''){
			console.log('hotloader加载失败：无端口号!');
			return;
		}
		//创建socket连接
        socket = new WebSocket("ws://localhost:"+ this.config.arrow_hotLoaderPort +"/");
	...
	//处理热加载
	this.handleHotLoad = function(_hotLoadItem){
		var data = JSON.parse(_hotLoadItem);
	...
	//兼容AMD标准
	window.require = window.require = function(_u,_callBack,_syncGuid){ return window.arrowHotLoader.require(_u,_callBack,_syncGuid); };
	//引用模块
	window.using = window.using = function(_u,_callBack,_syncGuid){ return window.arrowHotLoader.require(_u,_callBack,_syncGuid); };
}
window.arrowHotLoader = new arrowHotLoader();
