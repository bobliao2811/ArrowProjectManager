...
class mvvm{
	constructor(_config){
		var config = {
			...
			created:function(){},
			//创建前回调
			create:function(_next){_next();},
			//销毁前回调
			beforeDistory:function(_next){_next();},
			//销毁后回调
			distory:function(){}
		}
	...
	init(){
		...
		this.config.create(function(){
			...
			self.initListener();
			//保存原有的html内容
			self.saveOrgHtmlText();
			//执行解析器，解析html数据执行数据绑定
	...
	compiler(_html){
		...
		//执行双括号绑定
		this.doubleBracketsBind(_html);

		//执行class-bind，attr-bind，style-bind三绑定
		this.bindCAS(_html);

		//执行if绑定
		this.findIfCommand(_html);

		//执行纯脚本绑定f-bind
		this.findfBindCommand(_html);
	...
	bindCAS(_html){
		...
		var bind = function(_sourceCommand,_type,_bindFunc,_item,_listenTag){
			var sourceCommand = _sourceCommand;
			var result = self.explainAttrsExp(_type,sourceCommand);
			//给每个绑定的值注册监听器
	...
	explainAttrsExp(_type,_string){
		...
		if(res !== null){
			result.isArrow = true;
			//开始剥离箭头运算符
			var expArr = _string.replace('->','❤').split('❤');
			...
			result.arrowResult.expResult = self.executeExpression(dis,isHasDataTag);
	...
	//解释用户的绑定命令
	//并执行绑定
	explainDataBindCommand(_command,_htmlNode,_description){
		var self = this;
		var cArr = _command.split(';');
		//循环用“;”分割开的多命令
	...
	//往命令监视器里插入新监视器
	addListenerInList(_singleCommand,_htmlNode,_description){
	...
	doubleBracketsBind(_htmlNode){
		...
		for(var i=0;i<childNodes.length;i++){
			...
			if(Number(child.nodeType) === 3){
				var text = child.data;
				//测试文本中是否含有双括号
				var bindNodes = text.match(this.cmdRegExpList.isDoubleBracketsBind);
				//如果绑定字段不等于空就实施绑定
				if(bindNodes !== null){
					//循环所有的绑定节点，依次添加绑定
					//这里的逻辑是
					...
					for(var j=0;j<bindNodes.length;j++){
						...
						if(!isExp.isExp){
							...
							for(var e=0;e<isExp.dataTagList.length;e++){
								var dIDataTag = isExp.dataTagList[e];
								//添加数据监视
	...
	findIfCommand(_html){
		...
		var ifNodes = [];
		ifNodes = _html.find('[if]');
		//排除for中内部的if
		ifNodes = this.findOuterAttrTag('for',ifNodes);
		//排除和for并列的if
		var newNodes = [];
		...
		for(var i=0;i<ifNodes.length;i++){
			...
			ifItem[0].sourceIfCommand = dataTag;
			//执行数据绑定
			this.bindIf(dataTag,'if:'+dataTag,ifItem );
			//移除if命令
	...
	//执行fbind
	findfBindCommand(_html){
		//绑定f-bind语句
		//算法的核心思想如下
		//如果此节点上同时绑定了for命令，便不作处理
		//如果此f-bind命令节点的上级包含for命令便不作处理
		var fNodes = [];
		fNodes = _html.find('[f-bind]');
		//排除for中内部的f-bind
		fNodes = this.findOuterAttrTag('for',fNodes);
		//排除和for并列的if
		var newNodes = [];
		...
		for(var i=0;i<fNodes.length;i++){
			...
			fItem[0].sourcefBindCommand = dataTag;
			//执行数据绑定
			this.bindFB(dataTag,'f-bind:'+dataTag,fItem );
			//移除if命令
	...
	//绑定f-bind
	bindFB(_dataTag,_command,_htmlNode){
	...
	//绑定if命令的节点
	bindIf(_dataTag,_command,_htmlNode){
	...
	//执行单个命令
	executeCommand(_singleCommand,_htmlNode,_description){
		var self = this;
		var cArr = _singleCommand.split(':');
		var command = cArr[0];
		var dataTag = cArr[1];
		//添加数据监视
		this.addListenerInList(_singleCommand,_htmlNode);
		//获得数据值
	...
	//为某html节点执行表达式
	executeExpForNode(_exp,_isUsingData,_htmlNode){
		var self = this;
	...
	//执行表达式
	executeExpression(_exp,_isUsingData){
		var self = this;
		window["___oneTimeEval"] = {};
		window["___oneTimeEval"]["self"] = self;
		//获得数据中的所有字段文本
	...
	isExpression(_dataTag){
		var result = {isExp:false,dataTagList:[]};
		//如果是表达式
	...
	//绑定样式处理
	bindClass(_dataTag,_command,_htmlNode){
		var self = this;
		var sourceCommand = _htmlNode.sourceCommand;
		var result = self.explainAttrsExp(0,sourceCommand);
		//判断是否为箭头表达式
		if(result.isArrow){
			//如果为箭头表达式就判断运算结果是否为真
	...
	//绑定行样式处理器
	bindStyle(_dataTag,_command,_htmlNode){
		var self = this;
		var sourceCommand = _htmlNode.sourceCommand;
		var result = self.explainAttrsExp(2,sourceCommand);
		//判断是否为箭头表达式
		if(result.isArrow){
			//如果为箭头表达式就判断运算结果是否为真
			if(result.arrowResult.expResult){
	...
	bindText(_dataTag,_command,_htmlNode,_description){
		var self = this;
		//绑定文本的话，就需要从原始文本信息中执行绑定
		...
		for(var j=0;j<bindNodes.length;j++){
			...
			var isExp = this.isExpression(dataTag);
			var data = '';
			var bTag = 'textContent';
			//ie8以下浏览器不支持.textContent
			//所以要测试一下
			try{
	...
	//处理数据的更改事件绑定
	bindEvn(_dataTag,_command,_htmlNode){
		var self = this;
	...
	//绑定attr
	bindAttr(_dataTag,_command,_htmlNode){
		var self = this;
		if(typeof _htmlNode.sourceCommand !== undefined){
			var sourceCommand = _htmlNode.sourceCommand;
			var result = self.explainAttrsExp(1,sourceCommand);
			//判断是否为箭头表达式
			if(result.isArrow){
				//如果为箭头表达式就判断运算结果是否为真
				if(result.arrowResult.expResult){
	...
	//绑定val处理
	bindVal(_dataTag,_command,_htmlNode,_singleCommand){
		var self = this;
	...
	//从有嵌套关系的数据名称中设置数据
	set(_dataTag,_value,_isFromControlSet){
		var self = this;
		if(_dataTag.indexOf('.') !== -1){
			var arr = _dataTag.split('.');
			var layer = 0;
			var dig = function(_data,_compData,_dArr){
	...
	distroy(){
		...
		this.beforeDistory(function(){
			self.stop();
			//销毁之后还原最原始的html内容
			self.htmlObject.replaceWith($(self.orgHtml));
			//执行销毁后回调函数
			self.distroy();
		});
	...
	//只能拿最外层的for-in
	//嵌套for-in要避开
	findOuterAttrTag(_forTag,_forinNodes){
		var newNodes = [];
		for(var i=0;i<_forinNodes.length;i++){
			var nItem = $(_forinNodes[i]);
			var findUpper = function(_node){
	...
	explainFor(_dataTag,_htmlNode){
		...
		htmlTemp = htmlObject[0].outerHTML;
		//放入替代元素
	...
	//绑定事件
	//用于mvvm模板片段的事件绑定
	bindModulEvents(_html,_currentData){
		var self = this;
		var elems = [];
		...
		var bindEvents = function(_eventBindCommand){
		...
		for(var i=0;i<elems.length;i++){
			var eitem = $(elems[i]);
			var eventBindCommand = eitem.attr('data-event');
			//如果是单个事件绑定
	...
	//检查某个字段并更新
	checkDataAndUpdate(_dataTag,_value,_htmlNode,_command){
		var self = this;
		for(var i in self.listenList){
			var dataTag = i.split(':')[1];

			//比如 list  和list.0.value 是一个东西，
	...
	update(){
		...
		for(var i in self.listenList){
			...
			var gvalue = this.getFromComp(dataTag);
