//<!--ArrowCommand:DoNotChangeSuffixName-->
/*
...
window._arrowExpress = function(_config){
    ...
    this.regStrList = {
        ...
        "usingInCssFile":/\@[ ]{0,}using[ ]{0,}\((\s|\S)*?\)[ ]{0,}\;{0,}/g,
        //判断是否为绝对路径或者相对协议路径
        "isAbsUrl":/^http\:\/\/|^https\:\/\/|^\/\//
    };
    //拉取资源
    ...
    this.nonasyncRequire = function(_u){
        ...
        if(sysCatche !== null){
            if(self.regStrList.isStyleFile.test(_u) === true && self.config.isAutoHandleCss){
                self.autoHandleCssFiles(_u,sysCatche);
            }
            return sysCatche;
        }
        ...
        window.define = (function(){
            ...
            self.catche[_u.split('?')[0]] = result;
        });
        window.__setDefine();

        //无论js还是其它文件，只要同步加载都是使用xhr请求,并且不能多个同时请求
        this.getNonSync(_u,function(_r){
            if(self.regStrList.isJavascript.test(_u) || self.regStrList.isDll.test(_u)){
                eval(_r);
            }else{
                result = _r;
            }
    ...
    //创建缓存
    this.makeCatch = function(_id,_content){
        if(typeof(_id) === 'string'){
            _id = _id.split('?')[0];
            //如果是js文件,说明要缓存的对象是模块
            if(self.regStrList.isJavascript.test(_id)||self.regStrList.isDll.test(_id)){
                //运行缓存
                var content = _content();
                //直接缓存返回值
                //等到取缓存的时候可能会取到两种东西
                //一种是还未实例化的模块对象也就是一个单纯的function此时只需要返回
                //另一种是带有依赖的模块，带有依赖的模块需要在取出缓存时再作处理
                self.catche[_id.split('?')[0]] = content;
            }else{
                //否则要缓存的对象可能是html或者css
                self.catche[_id.split('?')[0]] = _content;
            }
        }else{
            for(var i=0;i<_id.length;i++){
                var id = _id[i].split('?')[0];
                //如果是js文件,说明要缓存的对象是模块
                if(self.regStrList.isJavascript.test(id)||self.regStrList.isDll.test(_id)){
                    //运行缓存
                    var content = _content();
                    //直接缓存返回值
                    //等到取缓存的时候可能会取到两种东西
                    //一种是还未实例化的模块对象也就是一个单纯的function此时只需要返回
                    //另一种是带有依赖的模块，带有依赖的模块需要在取出缓存时再作处理
                    self.catche[id.split('?')[0]] = content;
                }else{
                    //否则要缓存的对象可能是html或者css
                    self.catche[id.split('?')[0]] = _content;
                }
            }
        }
    }
        ...
        if(object !== null){
            if(typeof object._a_isDepsModul !== 'undefined' && object._a_isDepsModul === true){
                ...
                if(self.regStrList.isStyleFile.test(_url) === true && self.config.isAutoHandleCss){
                    self.autoHandleCssFiles(_url,object,_callBack);
                }
                _callBack(object);
            }
            return;
    ...
    //解析版本控制
    this.requireVersionControl = function(ver){
        //如果版本号为空就使用空字符串
        if(ver === false|| ver === "false" || typeof ver === 'undefined'){
            return '';
        }

        //如果版本号为手动填写，就使用手动填写的版本号
        if(typeof ver === 'string' && ver !== ''){
            return '_=' + ver;
        }

        //如果版本号只是填写了一个true,就使用动态时间戳
        if(ver === true||ver === "true"){
            return '_=' + (+new Date).toString();
        }
        return '';
    };

    //url重写控制器
    this.requireRerootControl = function(url){
        if( typeof url === 'undefined'){return url;}
        var s = "?";
        if(url.indexOf('?') !== -1){
            s = "&";
        }
        var _var = self.requireVersionControl(this.config.version)
        if(_var!== ''){
            url += s + _var;
        }
        return url;
    ...
    this.makeJavascriptRequire = function(_url,_callBack,_syncGuid){
        ...
        if(typeof document.currentScript !== 'undefined'){
            ...
            try{
                ...
                script.async = 'async';
                script.src = self.requireRerootControl(_url);
                ...
                self.parallelRequireList[taskGuid] = {
                    callBack:function(_result){
                        var result = _result;
                        //将请求到得数据保存进缓存
                        self.catche[_url.split('?')[0]] = result;
            ...
            window.__setDefine();
            if(typeof window.define.__proto__ !== 'undefined'){
    ...
    this.makeXHRrequire = function(_url,_callBack,_syncGuid){
        ...
        var rObj = this.get(_url,function(_content){
            ...
            //否则其它文件一律当作文本输出
            if(self.regStrList.isJavascript.test(_url)||self.regStrList.isDll.test(_url)){
                ...
                if(typeof result !=='undefined' && typeof result.isRequireobj !=='undefined' && result.isRequireobj === true){
                    self.syncRequireList[result.syncGuid] = function(_result){
                        self.catche[_url.split('?')[0]] = _result;
                        _callBack(_result);
                    }
                    return;
                }
    ...
    this.autoHandleUsingInCssFiles = function(_url,_result,_callback){
        ...
        for(var i=0;i<usingList.length;i++){
            usingFileList.push(usingList[i].replace(/\@[ ]{0,}using[ ]{0,}\((\'|\"|\`)/,'').replace(/(\'|\"|\`)\)[ ]{0,}\;{0,}/,''));
        }
        //请求那些依赖文件
    ...
    this.define = function(){
        ...
        if(typeof _modulName !== 'undefined'){
            self.catche[_modulName.split('?')[0]] = resultObj;
        }

        
        //如果浏览器支持document.currentScript
        //并且当前js请求模式为script节点加载模式
        if(typeof document.currentScript !== "undefined" && self.config.javascriptXHR === false && window.____loadMakeCatch === false){
        ...
        window.makeXHRrequireSyncResult = resultObj;

        return resultObj;
    }

    //设置系统兼容AMD语法
    if(typeof this.define.__proto__ !== 'undefined'){
    ...
    this.post = function (url, successCall, failedCall) {
        ...
        var ajaxobj = this.ajax(self.requireRerootControl(_url),
           { requestType: 'POST',
               asynch: true,
               sendData: {},
               type: "text",
    ...
    this.get = function (url, successCall, failedCall) {
        if (!successCall) { successCall = function () { }; }
        if (!failedCall) { failedCall = function () { }; }
        var ajaxobj = this.ajax(self.requireRerootControl(url),
           { requestType: 'GET',
               asynch: true,
               sendData: {},
               type: "text",
...
//是否在makeCatch状态
//避免makeCatch中define误将目前正在加载的任务删除
window.____loadMakeCatch = false;
//将组件向外部暴露
...
//创建缓存
var makeCatch = window.makeCatch = function(_id,_content){ window.arrowExpressjs.makeCatch(_id,_content); }
//创建缓存示例
//mackCatch('aaa/aaa/aaa.js',function(){
//     return define(['//www.aaa.com/common/common.css'],function(){
//        m=function(){
//            //...
//        }
//        return m;
...
//设置系统兼容AMD语法
window.__amddefineProp__ = {};
window.__setDefine = function(){
    if(typeof window.define.__proto__ !== 'undefined'){
        window.define.__proto__.amd= __amddefineProp__;
    }else{
        window.define.amd= __amddefineProp__;
    }
}
__setDefine();
/*
    ...
    if(typeof window.console === 'undefined'){
        window.console = {
            ...
            log:function(_message){
                window.console.messages.push(_message);
            }
        }
    }
    /*
...
zZ(document).ready(function(){
    /*
    设置热调试
    如果url中有ArrowHotDebugger=true的话就加载热调试模块
    */
    if(window.arrowExpressjs.getUrlParams()['ArrowHotDebugger'] === 'true'){
        using('arrowSystem/arrowHotDebugger.js');
    }
});
